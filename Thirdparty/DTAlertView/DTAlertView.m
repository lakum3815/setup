...
- (void)setButtonBackgroundColor:(UIColor *)buttonBackgroundColor
{
    if (_buttonBackgroundColor != nil) {
        DTRelease(_buttonBackgroundColor);
    }
    
    if (buttonBackgroundColor != nil) {
        _buttonBackgroundColor = DTRetain(buttonBackgroundColor);
    } else {
        _buttonBackgroundColor = [[UIColor alloc] initWithRed:33.0f/255.0f green:113.0f/255.0f blue:229.0f/255.0f alpha:1];
    }
    if (_cancelButtonTitle != nil) {
        UIButton *cancelButton = (UIButton *)[self viewWithTag:_cancelButtonIndex + 1];
        [cancelButton setBackgroundColor:[[UIColor alloc] initWithRed:33.0f/255.0f green:113.0f/255.0f blue:229.0f/255.0f alpha:1]];
    }
...
- (void)show
{
    ...
    [self showWithAnimation:DTAlertViewAnimationSlideBottom];
}
...
- (void)setViews
{
    ...
    [titleLabel setTextColor:[UIColor blackColor]];
     //[UIColor colorWithRed:43.0/255.0 green:179.0/255.0 blue:138.0/255.0 alpha:1.0]];
     //[UIColor colorWithRed:(float)66 / 255.0 green:(float)204/ 255.0 blue:(float)170 / 255.0 alpha:1.0]];
    ...
    [titleLabel setCenter:CGPointMake(CGRectGetMidX(self.bounds), titleLabel.center.y + 15.0f)];
    //[titleLabel setCenter:CGPointMake(CGRectGetMidX(self.bounds), titleLabel.center.y + 20.0f)];

    [self addSubview:titleLabel];
    ////NSLog@"DTAlerview Frame -> Width : %f - Height : %f",self.frame.size.width,self.frame.size.height);
//    UIImageView *imgBack = [[UIImageView alloc]initWithFrame:self.frame];
//    [imgBack setImage:[UIImage imageNamed:@"forgotpass_bg"]];
//    [imgBack setContentMode:UIViewContentModeScaleAspectFit];
//    [self insertSubview:imgBack atIndex:0];
#ifdef DEBUG_MODE
...
//     [UIColor colorWithRed:(float)73/ 255.0 green:(float)73/ 255.0 blue:(float)73 / 255.0 alpha:1.0]];
    [messageLabel setTextAlignment:NSTextAlignmentCenter];
    [messageLabel setFont:[UIFont fontWithName:@"Montserrat-Regular" size:15]];
    ...
    [buttonBackgroundView setBackgroundColor:[UIColor clearColor]];
    ...
    if (_cancelButtonTitle != nil) {
        ...
        [cancelButton setTitleColor:[UIColor whiteColor] forState:UIControlStateDisabled];
        [cancelButton setBackgroundColor:_buttonBackgroundColor];
//         [UIColor colorWithRed:(float)28/ 255.0 green:(float)158/ 255.0 blue:(float)118 / 255.0 alpha:1.0]];
    ...
    if (_positiveButtonTitle != nil) {
        ...
        [positiveButton setBackgroundColor:_buttonBackgroundColor];
         //[UIColor colorWithRed:(float)28/ 255.0 green:(float)158/ 255.0 blue:(float)118 / 255.0 alpha:1.0]];
        [positiveButton setTitleColor:[UIColor whiteColor] forState:UIControlStateDisabled];
        [positiveButton setTag:_cancelButtonIndex + 2];
        [positiveButton.titleLabel setFont:[UIFont fontWithName:@"Montserrat-SemiBold" size:16]];
        [positiveButton.titleLabel setNumberOfLines:0];
        [positiveButton.titleLabel setTextAlignment:NSTextAlignmentCenter];
...
- (UIButton *)setButtonWithTitle:(NSString *)buttonTitle
{
    ...
    [button setTitleColor:_buttonTextColor forState:UIControlStateHighlighted];
    [button.titleLabel setFont:[UIFont fontWithName:@"Montserrat-SemiBold" size:17]];
...
- (IBAction)buttonClicked:(UIButton *)sender
{
    ...
    if ((sender.tag - 1) != _cancelButtonIndex && _showForInputPassword) {
        ...
        dispatch_after(delayTime, dispatch_get_main_queue(), ^(void){
            [sender setEnabled:self->_showForInputPassword];
        });
    }
