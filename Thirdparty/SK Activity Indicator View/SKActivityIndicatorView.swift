...
public class SKActivityIndicator: NSObject {
    ...
    fileprivate var statusLabelFont        : UIFont                 = UIFont.systemFont(ofSize: 16)
    fileprivate var statusTextColor        : UIColor                = UIColor.black
    fileprivate var spinnerColor           : UIColor                = AppColor.PRIMARY
    fileprivate var backgroundViewColor    : UIColor                = UIColor.init(red  : 0.0, green  : 0.0, blue  : 0.0, alpha  : 0.4)
...
//            activityIndicatorView = UIToolbar(frame: CGRect.zero)
            activityIndicatorView = UIView(frame: CGRect.zero)

            activityIndicatorView!.layer.cornerRadius = 8
            activityIndicatorView!.layer.masksToBounds = true
           // activityIndicatorView!.isTranslucent = false
            
//            activityIndicatorView!.tintColor    = backgroundViewColor
            activityIndicatorView!.backgroundColor = UIColor.clear
//            activityIndicatorView!.barTintColor = backgroundViewColor
        ...
        if spinnerContainerView == nil {
            let spinnerViewFrame = CGRect(x: 0, y: 0, width: 65, height: 65)
        ...
        if spinnerContainerView != nil {
            ...
            let animationRectsize = CGSize(width: 65, height: 65)
    ...
    fileprivate func configureActivityIndicatorSize() {
        ...
        if let statusMessage =  statusLabel?.text, statusMessage.count != 0
        {
            ...
            rectLabel = (statusLabel?.text?.boundingRect(with: CGSize(width: 200, height: 300), options: options, attributes: attributes as [NSAttributedString.Key : Any] , context: nil))!
